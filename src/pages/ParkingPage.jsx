...
export default function ParkingPage() {
  return (
    <div className="flex min-h-dvh flex-col bg-background-dark text-slate-100">
      {/* Header */}
      <header className="sticky top-0 z-50 flex items-center justify-between border-b border-slate-800 bg-background-dark/80 px-4 py-3 backdrop-blur-md">
        <Link
          to="/"
      ...
      <main className="flex-1 space-y-8 overflow-y-auto px-4 py-6 pb-24">
        ...
        <section>
          ...
          <div className="no-scrollbar -mx-4 flex snap-x snap-mandatory gap-4 overflow-x-auto px-4 pb-2">
            {parkingLevels.map((p) => (
              <div
                key={p.level}
                className={`flex w-[160px] shrink-0 snap-center flex-col gap-3 rounded-2xl border border-slate-700 bg-surface-dark p-5 shadow-sm ${
              ...
              >
                ...
                <div className="mt-auto h-1.5 w-full overflow-hidden rounded-full bg-slate-700">
                  <div
        ...
        <section className="relative">
          ...
          <div className="relative z-10 rounded-2xl border border-primary/20 bg-surface-dark p-6 shadow-lg shadow-primary/5">
            ...
            <div className="mb-6 grid grid-cols-3 gap-3">
              {[
                ...
                <div
                  key={item.label}
                  className="rounded-xl border border-slate-700 bg-background-dark p-3 text-center"
                >
            ...
            {/* Photo */}
            <div className="group relative mb-6 h-32 w-full cursor-pointer overflow-hidden rounded-xl border border-slate-600 bg-slate-700">
              <div
        ...
        <section className="px-2">
          <div className="rounded-2xl border border-dashed border-slate-700 bg-white/5 p-6 text-center">
            <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-slate-700">
              <Icon name="edit_location" className="text-slate-400" />
            </div>
            ...
            <button className="rounded-full border border-slate-600 bg-slate-800 px-6 py-2.5 text-sm font-bold text-white transition-colors hover:bg-slate-700">
              Update Location
            </button>
          </div>
        </section>
      ...
      {/* Bottom Navigation */}
      <nav className="fixed bottom-0 w-full max-w-[390px] border-t border-slate-800 bg-background-dark px-6 pb-[env(safe-area-inset-bottom,20px)] pt-2">
        <div className="flex items-end justify-between pb-4">
          <Link to="/" className="group flex flex-col items-center gap-1 text-slate-400 transition-colors hover:text-white">
            <Icon name="home" className="text-[24px] transition-transform group-hover:-translate-y-1" />
            <span className="text-[10px] font-medium">Home</span>
          </Link>
          <Link to="/promos" className="group flex flex-col items-center gap-1 text-slate-400 transition-colors hover:text-white">
          ...
          <Link to="/parking" className="relative -top-4 flex flex-col items-center gap-1 text-primary">
            <div className="transform rounded-full border-4 border-background-dark bg-primary p-3 text-white shadow-lg shadow-primary/40 transition-transform hover:scale-110">
